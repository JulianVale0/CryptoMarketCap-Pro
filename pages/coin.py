...
def get_detail(cid):
    ...
    try:
        r = requests.get(url, params={"localization": False, "market_data": True, "sparkline": True}, headers=headers, timeout=10)
        r.raise_for_status()
        return r.json()
    except:
        return None
...
st.markdown(
    f"<h2 style='margin: 16px 0 0 0; font-size: 2.2rem;'>${price:,.4f} <span class='{cls}'>{change:+.2f}%</span></h2>",
    unsafe_allow_html=True,
)
