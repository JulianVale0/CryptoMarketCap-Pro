...
st.markdown("<style>@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap'); body{font-family:'Inter',sans-serif;}</style>", unsafe_allow_html=True)

st.markdown("""
<style>
    .main {background: linear-gradient(135deg, #0a0a1a 0%, #1a1a2e 100%);}
    .glass-card {background: rgba(30,35,60,0.7); backdrop-filter: blur(16px); border-radius: 20px; border: 1px solid rgba(0,212,170,0.2); padding: 20px; margin: 16px 0; box-shadow: 0 12px 32px rgba(0,0,0,0.4);}
...
st.markdown("<a href='/' class='back-btn'>← Back</a><br><br>", unsafe_allow_html=True)
...
def get_detail(cid):
    ...
    try: return requests.get(url, params={"localization":False, "market_data":True, "sparkline":True}, timeout=15).json()
    except: return None
...
def get_ohlc(cid):
    ...
    try:
        d = requests.get(url, params={"vs_currency":"usd","days":7}, timeout=15).json()
...
with c1: st.metric("Market Cap", f"${cap/1e9:.2f}B")
with c2: st.metric("Volume", f"${vol/1e6:.1f}M")
with c3: st.metric("ATH", f"${ath:,.2f}")
with c4: st.metric("ATL", f"${atl:,.4f}")
...
if not ohlc.empty:
    ...
    fig = go.Figure(data=[go.Candlestick(x=ohlc['ts'], open=ohlc['open'], high=ohlc['high'], low=ohlc['low'], close=ohlc['close'])])
